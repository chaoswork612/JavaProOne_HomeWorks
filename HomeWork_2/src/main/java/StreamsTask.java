...
public class StreamsTask {
    public static void main(String[] args) {
        ...
        System.out.println(numbers.stream().sorted(Comparator.comparing(Integer::intValue).reversed())
        ...
        System.out.println(numbers.stream().distinct().sorted(Comparator.comparing(Integer::intValue).reversed())
                .skip(2).findFirst().orElseThrow());
        ...
        listOfEngineers.stream().sorted(Comparator.comparing(Employee::getAge).reversed())
                .filter(employee -> employee.getPosition().equals("Инженер"))
                .limit(3).forEach(System.out::println);
        /*Имеется список объектов типа Сотрудник (имя, возраст, должность),
        посчитайте средний возраст сотрудников с должностью «Инженер»*/
        listOfEngineers.stream().filter(employee -> employee.getPosition().equals("Инженер"))
                .mapToInt(Employee::getAge).average().ifPresent(System.out::println);
        /*Найдите в списке слов самое длинное*/
        var wordList = List.of("Test", "TestTest", "TestTestTestTest",
                "TestTestTestTestTestTest", "TestTestTestTestTestTestTestTest", "BestTestTestTestTestTestTestTest");
        wordList.stream().max(Comparator.comparing(String::length)).ifPresent(System.out::println);
        ...
        System.out.println(Stream.of(string.toLowerCase().split(" "))
        ...
        wordList.stream().sorted((s1, s2) ->
                        s1.length() == s2.length() ? s1.compareTo(s2) : Integer.compare(s1.length(), s2.length()))
        ...
        var listOfWordsList = List.of(List.of("Test", "TestTest", "TestTestTestTest",
        ...
        listOfWordsList.stream().flatMap(List::stream)
